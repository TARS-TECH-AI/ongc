...
const StatusBadge = ({ status }) => {
  ...
  if (status === "Approved")
    return <span className={`${base} bg-sky-900 text-white`}>{status}</span>;
  if (status === "Rejected")
    return <span className={`${base} bg-rose-600 text-white`}>{status}</span>;
  return (
    <span className={`${base} bg-amber-300 text-amber-900`}>{status}</span>
  );
};
...
const Approvals = () => {
  ...
  const loadRows = async (status = "") => {
    ...
    try {
      ...
      const res = await fetch(`${API}/admin/approvals${queryParam}`, {
      ...
      // Ensure newest registrations appear first (client-side fallback)
      ...
      const mapped = data.map((u) => ({
        ...
        date: new Date(u.date || u.createdAt).toLocaleDateString(),
        status: u.status,
        category: u.category,
  ...
  const changeStatus = async (id, newStatus) => {
    ...
    try {
      ...
      // revert on failure
      setRows((prev) =>
        prev.map((r) => (r.id === id ? { ...r, status: old } : r))
      );
  ...
  return (
    <div className="min-h-screen bg-slate-50 py-6">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {/* Desktop table */}
        {loadingRows ? (
          <div className="bg-white rounded-lg shadow p-6 text-center text-sm text-slate-600">
            Loading approvals...
          </div>
        ) : (
            ...
            <table className="min-w-full divide-y">
              <thead>
                <tr className="bg-white">
                  ...
                  <th className="px-6 py-3 text-right text-xs font-bold text-slate-900 uppercase tracking-wider">
                    Actions
                  </th>
                </tr>
              </thead>
            ...
            >
              <div>
                ...
                <div className="text-xs text-slate-500">
                  {row.employeeId || "-"} • {row.category || "-"} •{" "}
                  {row.designation || "-"} • {row.date}
                </div>
              </div>
                  ...
                  <div className="border-t pt-6 mb-6">
                            ...
                            <div className="flex items-center gap-3">
                              ...
                              <div>
                                ...
                                <div className="text-sm text-slate-500">
                                  {selected.idProofFileType || "Document"}
                                </div>
                              </div>
                            </div>
                            <div className="flex gap-2">
                              <button
                                onClick={() =>
                                  viewDocument(
                                    selected.idProofDocument,
                                    selected.idProofFileName || "ID Proof"
                                  )
                                }
                    ...
                    <div>
                      ...
                      <div className="space-y-2">
                        {selected.docs && selected.docs.length ? (
